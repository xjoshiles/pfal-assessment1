...
const SetPreview = ({
  ...
  onDeleteSet: (id: number) => void  // on prefix convention for props
}) => {
  const { id, name, description, userId, averageRating, creator, updatedAt } = set
    ...
    <li className='item_preview group overflow-x-auto scrollbar-hidden'>
      ...
      {showDeleteModal && (
        <div role="dialog" className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          ...
          >
            <h2 className="text-lg font-bold text-red-600">Delete Set</h2>
            <p className="text-sm text-gray-600">
              Are you sure? This action can't be reversed, and any reviews and flashcards will be deleted.
            </p>
